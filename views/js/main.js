...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
      var pcont = document.getElementsByClassName("randomPizzaContainer");
      var dx = determineDx(pcont[0], size);
      var newwidth = (pcont[0].offsetWidth + dx) + 'px';
...
function updatePositions() {
  ...
  for (var i = 0; i < items.length; i++) {
    phase = Math.sin((scrollToTop / 1250) + (i % 5));
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
    // Hardcoded value to reduce number of loops
  for (var i = 0; i < 45; i++) {
